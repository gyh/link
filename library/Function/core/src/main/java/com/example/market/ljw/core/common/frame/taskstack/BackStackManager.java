...
public class BackStackManager {
    ...
    public TaskModule pop() {
        if (prev() == null)
            return null;
        this.current = this.history.remove(-1+ this.history.size());
        return this.current;
    }
    ...
    public TaskModule prev() {
        if (isLast())
            return null;
        return this.history.get(-1 + this.history.size());
    }
    ...
    public void tryPushPrev(TaskModule preTaskModule) {
        if ((preTaskModule != null) && ((preTaskModule instanceof NeedShowAgainModule))) {
            push(this.prev);
        }
    }
}
