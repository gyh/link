...
public class SecurityUtil
{
     ...
     * 向参数内追加时间戳参数
     */
    public static Map processParam(Map paramMaps){
        Object[] keySet = paramMaps.keySet().toArray();
        String t = System.currentTimeMillis()/1000 + "";
        for(int i=0;i<keySet.length;i++){
            String key = keySet[i].toString();
            if("r".equals(key)){
                //key 与 r路径 与时间的md5摘要，得到新的vcode值
                String vcode = getMD5((Constant.KEY + paramMaps.get(key) + t));
                paramMaps.put("vcode", vcode);
                paramMaps.put("t", t);
            }
        }
        return paramMaps;
