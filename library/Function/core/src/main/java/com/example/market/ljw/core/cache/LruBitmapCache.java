...
import com.example.market.ljw.core.cache.util.LruCache;
...
public class LruBitmapCache implements CacheManager {
	...
	private void reset() {
		...
		cache = new LruCache<GlobalImageCache.BitmapDigest, Bitmap>(capacity) {
			...
			@Override
			protected long sizeOf(GlobalImageCache.BitmapDigest bitmapDigest, Bitmap bitmap) {
				if (Log.D)
					Log.d(LruBitmapCache.class.getName(),
	...
	@Override
	public void put(GlobalImageCache.BitmapDigest bitmapDigest, Bitmap bmp) {
